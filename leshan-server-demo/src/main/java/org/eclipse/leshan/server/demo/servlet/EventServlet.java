    ...
    private final ObservationRegistryListener observationRegistryListener = new ObservationRegistryListener() {
        ...
        public void newValue(Observation observation, ObserveResponse response) {
            ...
            if (client != null) {
                ...
                /*
                 * // List<TimestampedLwM2mNode> obresp = response.getTimestampedLwM2mNode(); JsonObject jsonObject =
                 * new JsonParser().parse(gson.toJson(response.getContent())).getAsJsonObject();
                 * 
                 * // System.out.println(jsonObject.get("id").getAsString()); DateFormat dateFormat = new
                 * SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); java.util.Date date = new java.util.Date(); String timestamp
                 * = dateFormat.format(date);
                 * 
                 * try {
                 * 
                 * MongoClient mongoClient = new MongoClient("localhost", 27017); MongoDatabase database =
                 * mongoClient.getDatabase("qolsys"); MongoCollection<Document> collection =
                 * database.getCollection("events"); String event = jsonObject.get("value").getAsString().trim();
                 * Document document = new Document(); document.put("client_ep", client.getEndpoint());
                 * document.put("event", event); document.put("timestamp", timestamp); collection.insertOne(document);
                 * mongoClient.close();
                 * 
                 * } catch (Exception e) { e.printStackTrace(); System.err.println(e.getClass().getName() + ": " +
