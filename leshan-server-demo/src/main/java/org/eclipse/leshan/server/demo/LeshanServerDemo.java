...
import org.eclipse.leshan.server.demo.servlet.CliServlet;
import org.eclipse.leshan.server.demo.servlet.ClientServlet;
import org.eclipse.leshan.server.demo.servlet.EventServlet;
import org.eclipse.leshan.server.demo.servlet.FetchServlet;
import org.eclipse.leshan.server.demo.servlet.ObjectSpecServlet;
import org.eclipse.leshan.server.demo.servlet.SampleServlet;
...
public class LeshanServerDemo {
    ...
    public static void createAndStartServer(int webPort, String localAddress, int localPort, String secureLocalAddress,
        ...
        // Now prepare Jetty Server server = new Server(webPort); WebAppContext root = new WebAppContext();
        // root.setContextPath("/");
        // root.setResourceBase(LeshanServerDemo.class.getClassLoader().getResource("webapp").toExternalForm());
        // root.setParentLoaderPriority(true);
        // server.setHandler(root);
        ...
        ServletHolder FetchServletHolder = new ServletHolder(new FetchServlet());
        root.addServlet(FetchServletHolder, "/api/fetch/*");

        // Suresh
        ServletHolder SampleServletHolder = new ServletHolder(new SampleServlet());
        root.addServlet(SampleServletHolder, "/api/test/*");
        ...
        ServletHolder cliServletHolder = new ServletHolder(
                new CliServlet(lwServer, lwServer.getSecureAddress().getPort()));
        root.addServlet(cliServletHolder, "/api/cli/*");
        // Start Jetty & Leshan
        lwServer.start();
        server.start();
